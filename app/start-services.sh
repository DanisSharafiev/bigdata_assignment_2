...
# Wait for slave SSH to be ready
echo "Waiting for cluster-slave-1 SSH..."
until ssh -o StrictHostKeyChecking=no -o ConnectTimeout=2 cluster-slave-1 echo ok 2>/dev/null; do
    sleep 2
done
echo "cluster-slave-1 SSH ready"
